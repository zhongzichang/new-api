...
func GetTopUpInfo(c *gin.Context) {
	...
	// 如果enabled了 Stripe payment，添加到payment方法列表
	...
	// 如果enabled了 Waffo payment，添加到payment方法列表
	...
	data := gin.H{
		...
		"enable_usdc_topup":                isUsdcTopUpEnabled(),
		...
		"usdt_base_receiver":      setting.UsdtBaseReceiver,
		"usdt_polygon_receiver":   setting.UsdtPolygonReceiver,
		"usdt_tron_receiver":      setting.UsdtTronReceiver,
		"usdt_eth_contract":       setting.UsdtEthContract,
		"usdt_bsc_contract":       setting.UsdtBscContract,
		"usdt_base_contract":      setting.UsdtBaseContract,
		"usdt_polygon_contract":   setting.UsdtPolygonContract,
		"usdt_tron_contract":      setting.UsdtTronContract,
		"usdt_eth_decimals":       setting.UsdtEthDecimals,
		"usdt_bsc_decimals":       setting.UsdtBscDecimals,
		"usdt_base_decimals":      setting.UsdtBaseDecimals,
		"usdt_polygon_decimals":   setting.UsdtPolygonDecimals,
		"usdt_tron_decimals":      setting.UsdtTronDecimals,
		"usdt_min_topup":          setting.UsdtMinTopUp,
		"usdt_unit_price":         setting.UsdtUnitPrice,
		"usdt_timeout_minutes":    setting.UsdtTimeoutMinutes,
		"usdc_eth_receiver":       setting.UsdcEthReceiver,
		"usdc_bsc_receiver":       setting.UsdcBscReceiver,
		"usdc_base_receiver":      setting.UsdcBaseReceiver,
		"usdc_polygon_receiver":   setting.UsdcPolygonReceiver,
		"usdc_tron_receiver":      setting.UsdcTronReceiver,
		"usdc_eth_contract":       setting.UsdcEthContract,
		"usdc_bsc_contract":       setting.UsdcBscContract,
		"usdc_base_contract":      setting.UsdcBaseContract,
		"usdc_polygon_contract":   setting.UsdcPolygonContract,
		"usdc_tron_contract":      setting.UsdcTronContract,
		"usdc_eth_decimals":       setting.UsdcEthDecimals,
		"usdc_bsc_decimals":       setting.UsdcBscDecimals,
		"usdc_base_decimals":      setting.UsdcBaseDecimals,
		"usdc_polygon_decimals":   setting.UsdcPolygonDecimals,
		"usdc_tron_decimals":      setting.UsdcTronDecimals,
		"usdc_min_topup":          setting.UsdcMinTopUp,
		"usdc_unit_price":         setting.UsdcUnitPrice,
		"usdc_timeout_minutes":    setting.UsdcTimeoutMinutes,
...
func RequestEpay(c *gin.Context) {
	...
	if client == nil {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Payment information not configured by admin"})
		return
	}
...
// UnlockOrder 释放给定order number的锁
...
func EpayNotify(c *gin.Context) {
	...
	if len(params) == 0 {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Epay webhook parameters empty path=%q client_ip=%s", c.Request.RequestURI, c.ClientIP()))
		_, _ = c.Writer.Write([]byte("fail"))
		return
	}
	client := GetEpayClient()
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.PaymentProvider != model.PaymentProviderEpay {
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("Epay Order paid gateway mismatch trade_no=%s order_provider=%s callback_type=%s client_ip=%s", verifyInfo.ServiceTradeNo, topUp.PaymentProvider, verifyInfo.Type, c.ClientIP()))
			return
		}
		if topUp.Status == common.TopUpStatusPending {
			...
			model.RecordTopupLog(topUp.UserId, fmt.Sprintf("Online top-up successful, amount: %v, payment: %f", logger.LogQuota(quotaToAdd), topUp.Money), c.ClientIP(), topUp.PaymentMethod, "epay")
		}
	} else {
...
// GetAllTopUps 管理员获取全平台Top-up recorded
