...
func SubscriptionRequestCreemPay(c *gin.Context) {
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem subscription Failed to read payment request error=%q", err.Error()))
	...
	if plan.CreemProductId == "" {
		common.ApiErrorMsg(c, "该套餐未configuration CreemProductId")
		return
	}
	if setting.CreemWebhookSecret == "" && !setting.CreemTestMode {
		common.ApiErrorMsg(c, "Creem Webhook 未configuration")
		return
	}
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem subscription payment link Failed to create trade_no=%s product_id=%s error=%q", referenceId, product.ProductId, err.Error()))
